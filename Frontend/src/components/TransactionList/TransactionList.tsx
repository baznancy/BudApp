...
const TransactionList: React.FC = () => {
  ...
  const [filters, setFilters] = useState<{ startDate: string; endDate: string; type: string }>({
    startDate: "",
    endDate: "",
    type: "",
  });
  ...
  const fetchTransactions = async (filters: { startDate: string; endDate: string; type: string }) => {
    const query = `
      query FilteredTransactions($startDate: String, $endDate: String, $type: String) {
        filteredTransactions(startDate: $startDate, endDate: $endDate, type: $type) {
          id
          amount
          type
          tags
    ...
    `;
    try {
      const response = await graphqlClient(query, filters);
      const transactionsData = response?.data?.filteredTransactions || [];
  ...
  // Handle updates to the filter state
  const handleFilter = (newFilters: { startDate: string; endDate: string; type: string }) => {
  ...
  return (
    <div className={styles["transaction-list"]}>
      ...
      <TransactionFilter onFilter={handleFilter} /> {/* Pass handleFilter to the filter component */}
